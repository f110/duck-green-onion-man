...
BEGIN {
    use_ok "App::Onion::Parser", qw/get_body/;
}

my $html = do { local $/; <DATA> };
my $expected_message_body = <<"__MESSAGE_BODY__";
今年もmixi Xmasの季節がきたよ！一緒に幸運のベルを鳴らして、クリスマスまでのカウントダウンを楽しもう！プレゼントや、無料のクーポン券がもらえるよ♪

☆mixi Xmas 2012☆
http://mixixmas.mplace.jp/?from=miv
__MESSAGE_BODY__

subtest "get_body" => sub {
    my $message_body = get_body($html);
    is $message_body, $expected_message_body;
};

subtest "get_body but couldn't get html" => sub {
    my $message_body = get_body("");
    is $message_body, undef;
};

subtest "get_body but get an other dom tree something like error page" => sub {
    my $message_body = get_body("<html><body>error</body></html>");
    is $message_body, undef;
};
